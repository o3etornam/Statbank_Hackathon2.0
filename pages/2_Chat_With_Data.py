...
viewer = st.selectbox('Which dataset will you like to view', datasets.keys())
with st.expander('Click to view dataset'):
    st.dataframe(datasets[viewer])
...
@st.cache_resource
def query_df(prompt,dfs = datasets.values(),llm = llm):
    dl = SmartDatalake(dfs, config={"llm": llm})
    if prompt:
        return dl.chat(prompt)
